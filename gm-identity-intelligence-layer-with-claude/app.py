"""
GM Identity Intelligence — Multi-Region Flask Server
Supports US, MEO, Mexico tenants with region switching.
"""
...
@app.route("/agentic")
def agentic():
    return send_from_directory("static", "dashboard-agentic.html")
...
# ── B2B Fleet Fragmentation Scanner ──────────────────────────────────────────

@app.route("/api/fleet-fragmentation")
def fleet_fragmentation():
    mn = int(request.args.get("min", 10))
    lim = int(request.args.get("limit", 50))
    return jsonify(get_api().get_fleet_fragmentation(mn, lim))
...
def fleet_fragmentation_count():
    mn = int(request.args.get("min", 10))
    return jsonify({"total": get_api().get_fleet_fragmentation_count(mn)})
...
if __name__ == "__main__":
    ...
    print("\n" + "═" * 60)
    print("  GM IDENTITY INTELLIGENCE — Multi-Region")
    print("═" * 60)

    init_regions()

    # Initialize AI Engine (Claude/Gemini)
    port_for_ai = find_open_port()
    ...
    port = port_for_ai
    AI_ENGINE.base_url = f"http://127.0.0.1:{port}"
    ...
    print(f"  Agentic:          http://127.0.0.1:{port}/agentic")
