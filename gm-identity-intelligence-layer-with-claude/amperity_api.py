"""
Amperity Transit+JSON Query Layer — Multi-Region GM Support
...
def load_regions():
    """Load all regions from .env. Supports REGION_US_*, REGION_MEO_*, REGION_MX_* prefixes."""
...
class AmperityAPI:
    ...
    def run_query(self, sql, limit=500, timeout=60):
        ...
        transit_body = json.dumps([
            ...
            "~:amperity.query.exec/description", "GM Identity Engine Query",
    ...
    def search_customer(self, q):
        ...
        if re.match(r'^[0-9a-f]{8}-', q, re.I):
            ...
            return self.run_query(f"""
                SELECT DISTINCT amperity_id, given_name, surname, email, phone, city, state
                FROM Unified_Coalesced
                WHERE UPPER(email) LIKE '%{q.upper()}%'
                LIMIT 20
            """)
    ...
    # ── B2B Fleet Fragmentation Scanner (MEO: the 122-node story) ────

    def get_fleet_fragmentation(self, min_records=10, limit=50):
        """Find clusters that look like business entities fragmented into consumer nodes.
        Signals: many records, many distinct addresses, few distinct names (company-like)."""
        return self.run_query(f"""
            SELECT amperity_id,
            ...
            GROUP BY amperity_id
            HAVING COUNT(*) >= {min_records}
               AND COUNT(DISTINCT COALESCE(address, '')) >= 5
            ORDER BY COUNT(*) DESC
            LIMIT {limit}
        """)

    def get_fleet_fragmentation_count(self, min_records=10):
        """Count total B2B-fragmented clusters (no LIMIT)."""
