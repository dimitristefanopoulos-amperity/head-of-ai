"""
Atlas Motors Identity Intelligence — Single-Region Flask Server
Single-tenant identity resolution explainability tool for Atlas Motors.
"""
...
AI_ENGINE = None
TENANT_AGENT = None     # Claude/Gemini AI engine
STRATEGIST = None        # Tenant strategist
...
# ── Strategist — Marketing & Identity Intelligence ────────────────────────
...
def strategist_quick():
    ...
    try:
        ...
        r = req.post("https://api.anthropic.com/v1/messages", headers={
            "x-api-key": claude_key, "content-type": "application/json", "anthropic-version": "2023-06-01"
        }, json={
            "model": "claude-sonnet-4-20250514", "max_tokens": 1024,
            "system": f"You are a marketing strategist for an Amperity tenant. Answer in 3-5 concise, actionable sentences. Never mention internal details, vendor names, or implementation notes. Frame everything as business opportunity.\n\n{ctx}",
...
# ── Startup ───────────────────────────────────────────────────────────────────
# ── In-Tenant Agentic Intelligence ────────────────────────────────────────

@app.route("/agentic")
def agentic_view():
    return send_from_directory("static", "agentic.html")

@app.route("/api/agentic/scan", methods=["POST"])
def agentic_scan():
    """Run full OBSERVE → DIAGNOSE → PROPOSE → GATE cycle."""
    global TENANT_AGENT
...
def agentic_execute():
    ...
    return jsonify({"error": "Proposal not found or not approved"}), 404

@app.route("/api/agentic/status")
def agentic_status():
    if TENANT_AGENT:
        return jsonify(TENANT_AGENT.get_status())
...
def agentic_policy():
    if TENANT_AGENT:
        return jsonify(TENANT_AGENT.policy.to_dict())
    return jsonify({})
...
if __name__ == "__main__":
    ...
    print("\n" + "═" * 60)
    print("  ATLAS MOTORS IDENTITY INTELLIGENCE")
    print("═" * 60)
    ...
    # Initialize In-Tenant Agentic Agent
    ...
    if STRATEGIST.available:
        ...
        print("  ⚠ Strategist disabled: set CLAUDE_API_KEY or GEMINI_API_KEY in .env")
    ...
    print(f"  Agentic Ops:     http://127.0.0.1:{port}/agentic")
