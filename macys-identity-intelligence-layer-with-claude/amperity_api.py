...
class AmperityAPI:
    ...
    def search_customer(self, q):
        ...
        Macy's merged_customer uses: AMPID, Fname, Lname, Email, Phone, City, State.
        Email search joins through Unified_Coalesced since merged_customer only keeps one email."""
        q = q.strip()
        q_upper = q.upper().replace("'", "''")
        if re.match(r'^[0-9a-f]{8}-', q, re.I):
            # AMPID
            return self.run_query(f"""
                SELECT DISTINCT AMPID AS amperity_id, Fname AS given_name, Lname AS surname,
                       Email AS email, Phone AS phone, City AS city, State AS state
                FROM merged_customer
                WHERE AMPID = '{q}'
                LIMIT 20
            """)
        elif '@' in q:
            # Email search — must check Unified_Coalesced because merged_customer
            # only keeps one "winning" email per AMPID. UC has ALL emails.
            return self.run_query(f"""
                SELECT DISTINCT mc.AMPID AS amperity_id, mc.Fname AS given_name,
                       mc.Lname AS surname, mc.Email AS email, mc.Phone AS phone,
                       mc.City AS city, mc.State AS state
                FROM Unified_Coalesced uc
                JOIN merged_customer mc ON uc.amperity_id = mc.AMPID
            ...
            if len(parts) >= 2:
                ...
                return self.run_query(f"""
                    SELECT DISTINCT AMPID AS amperity_id, Fname AS given_name, Lname AS surname,
                           Email AS email, Phone AS phone, City AS city, State AS state
                    FROM merged_customer
                    WHERE UPPER(Fname) LIKE '%{fn}%' AND UPPER(Lname) LIKE '%{ln}%'
                    LIMIT 20
                """)
            else:
                return self.run_query(f"""
                    SELECT DISTINCT AMPID AS amperity_id, Fname AS given_name, Lname AS surname,
                           Email AS email, Phone AS phone, City AS city, State AS state
                    FROM merged_customer
                    WHERE UPPER(Fname) LIKE '%{q_upper}%'
                       OR UPPER(Lname) LIKE '%{q_upper}%'
