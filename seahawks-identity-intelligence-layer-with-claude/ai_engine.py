...
class AIEngine:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:5080"):
        ...
        # Resolve tenant context from env
        tenant_name = ""
        for k, v in os.environ.items():
            if k.startswith("REGION_") and k.endswith("_NAME") and v:
                tenant_name = v
                break
        self.system_prompt = SYSTEM_PROMPT.replace("{tenant_context}", get_tenant_context(tenant_name))
    ...
    def _call_tool(self, name: str, args: dict) -> str:
        ...
        try:
            if name == "get_identity_health":
                r = requests.get(f"{self.base_url}/api/cross-region-pulse", timeout=30)
            elif name == "get_dedup_scorecard":
                r = requests.get(f"{self.base_url}/api/dedup-scorecard", timeout=30)
            elif name == "get_source_scorecard":
                r = requests.get(f"{self.base_url}/api/source-scorecard", timeout=30)
            elif name == "get_stitch_stats":
                r = requests.get(f"{self.base_url}/api/stitch-stats", timeout=30)
            elif name == "get_cluster_health":
                r = requests.get(f"{self.base_url}/api/cluster-health/distribution", timeout=30)
            elif name == "get_drift_alerts":
                r = requests.get(f"{self.base_url}/api/drift/alerts", timeout=30)
            elif name == "get_drift_history":
                r = requests.get(f"{self.base_url}/api/drift/history", timeout=30)
            elif name == "search_customer":
                r = requests.get(f"{self.base_url}/api/search", params={"q": args.get("query", "")}, timeout=30)
            elif name == "explain_cluster":
                ampid = args.get("amperity_id", "")
                r = requests.get(f"{self.base_url}/api/explain/{ampid}", timeout=60)
            elif name == "get_name_variants":
                r = requests.get(f"{self.base_url}/api/name-variants", timeout=30)
            elif name == "get_source_overlap":
                r = requests.get(f"{self.base_url}/api/source-overlap", timeout=30)
            else:
